...
Greenhouse / Lever / Ashby are enterprise sales-only — there's no self-serve developer tier we can use to exercise the publish loop end-to-end in dev or CI. This server mimics all three accurately enough that our adapters' `test_connection()`, `list_*()`, and `publish_job()` paths succeed without modification.
