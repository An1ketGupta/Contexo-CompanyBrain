...
Every part of the template pipeline that needs to point at a location in a
Word document — the blank detector, the persisted slot rows, the direct-fill
renderer, and the flat-text HR editor — addresses it as
`(paragraph_index, character offset)` where `paragraph_index` is a position in
the enumeration `canonical_paragraphs()` returns.
...
definition of both, extracted out of `template_analyzer` precisely so a second
consumer can't drift from the first: a slot detected at index 41 must resolve
to the same paragraph weeks later when a candidate's document is generated.
"""
...
def docx_hash(docx_bytes: bytes) -> str:
