    ...
    <div className="mx-auto max-w-2xl space-y-6 p-6 md:p-8">
      <header>
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          Tune when the chat confidence badge shows green / amber / red. The
          score is the average vector-similarity (0–10) of the chunks cited
          in the answer.
        </p>
      </header>
...
function ThresholdEditor({
  ...
  // Keep medium <= high. Cleaner to clamp on input than to error on submit.
  useEffect(() => {
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/admin/config/confidence-thresholds", {
        ...
        body: JSON.stringify({ high: high / 100, medium: medium / 100 }),
      });
      if (!res.ok) {
