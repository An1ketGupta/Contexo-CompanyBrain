 ...
 * is only consulting one document. On the new-chat surface the "Search all"
 * link clears the scope by replacing the URL; on persisted conversations
 * scope is fixed (we don't allow rescoping mid-thread).
 */
export function ScopeBanner({ documentName, clearHref }: ScopeBannerProps) {
  return (
    <div className="flex items-center gap-2 border-b border-blue-200 bg-blue-50 px-4 py-2 text-sm text-blue-900 dark:border-blue-900/60 dark:bg-blue-950/40 dark:text-blue-200">
      ...
      <span className="min-w-0 truncate">
        Asking only <strong className="font-medium">{documentName}</strong>
      </span>
      {clearHref && (
        <Link
          href={clearHref}
